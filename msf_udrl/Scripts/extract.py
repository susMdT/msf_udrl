...
if __name__ in '__main__':
    try:
        ...
        parser.add_argument( '-s', required = False, help = 'Section index. 0 = Text, 2 = Data', type = int );
        parser.add_argument( '-overlay',  required = False, help = 'Specify if going for the overlay', action='store_true')
        option = parser.parse_args();

        PeExe = pefile.PE( option.f );
        PeSec = PeExe.sections[ option.s ].get_data();
